...
import {
  ...
  processSalesCandidate,
  updateSalesCandidate,
...
import type { SalesCandidateRecord, SalesStage, SalesView } from '../types/sales';
import { writeRecruiterCandidates } from '../utils/recruiterStorage';
import { readSalesCandidates, writeSalesCandidates } from '../utils/salesStorage';

const stageOptions: SalesStage[] = ['TO DO', 'INTERVIEWING'];
...
type StageFilter = SalesStage | 'ALL';
...
const emptyProcessFormValues: ProcessFormState = {
  interviewLink: '',
  interviewSchedule: '',
};
...
const isTodoCandidate = (candidate: SalesCandidateRecord) => candidate.stage === 'TO DO';

const getStageClassName = (stage: SalesStage) =>
  `recruiter-stage-${stage.toLowerCase().replace(/\s+/g, '-')}`;

interface SalesCandidateModalProps {
        ...
        <form className="form-grid" onSubmit={onSubmit}>
          ...
          <label className="full-width">
            ...
            <textarea
              rows={4}
              name="summary"
              placeholder="Isi poin penting kandidat, pengalaman, skill utama, atau catatan sales."
          ...
          <div className="full-width recruiter-upload-grid">
            <label className="recruiter-upload-card">
              ...
              />
              <small>
                {uploads.cv ? uploads.cv.name : 'Biarkan kosong jika tidak ingin mengganti CV'}
              </small>
            </label>
...
interface SalesCandidateDetailModalProps {
  candidate: SalesCandidateRecord | null;
  canEdit: boolean;
  onClose: () => void;
  onEdit: () => void;
}

function SalesCandidateDetailModal({
  candidate,
  canEdit,
  onClose,
  onEdit,
}: SalesCandidateDetailModalProps) {
  if (!candidate) {
    return null;
  }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Detail kandidat sales"
      >
        <div className="panel-header modal-header">
          <div>
            <h2>Detail Kandidat Sales</h2>
            <p>Ringkasan lengkap data kandidat yang sudah diteruskan dari recruiter.</p>
          </div>
            ...
            <div className="recruiter-detail-edit-wrapper">
              ...
              {!canEdit ? (
                <div className="recruiter-edit-tooltip" role="tooltip">
                  Data yang sudah diproses sales tidak bisa diubah lagi.
                </div>
              ) : null}
            </div>
        ...
        <section className="recruiter-detail-hero">
          <div className="recruiter-photo-preview-card recruiter-detail-photo-card">
            <img className="recruiter-photo-preview-image" src={photoUrl} alt={candidate.fullName} />
          </div>

          <div className="recruiter-detail-summary">
            <div>
              <p className="eyebrow recruiter-detail-eyebrow">Sales Candidate</p>
        ...
        <section className="recruiter-detail-section">
          ...
          <div className="recruiter-detail-grid">
            ...
            <div className="recruiter-detail-item">
              <span>Masuk Sales Pada</span>
              <strong>{formatCreatedAt(candidate.createdAt)}</strong>
            </div>
            <div className="recruiter-detail-item">
              <span>Schedule Interview</span>
              <strong>{formatInterviewSchedule(candidate.interviewSchedule ?? '')}</strong>
            </div>
            <div className="recruiter-detail-item">
              <span>Link Interview</span>
              <strong>
                {candidate.interviewLink ? (
                  <a href={candidate.interviewLink} target="_blank" rel="noreferrer">
                    Open Link
                  </a>
                ) : (
                  '-'
        ...
        <section className="recruiter-detail-section">
          ...
          <div className="empty-block recruiter-detail-note">
            <p>{candidate.summary || 'Belum ada catatan sales.'}</p>
          </div>
        </section>
      </div>
    </div>
...
interface SalesCandidateProcessModalProps {
  candidate: SalesCandidateRecord | null;
...
function SalesCandidateProcessModal({
  candidate,
  processForm,
  onChange,
  onClose,
  onConfirm,
}: SalesCandidateProcessModalProps) {
  if (!candidate) {
    return null;
  }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Konfirmasi proses kandidat sales"
      >
        <div className="modal-header recruiter-process-modal-header">
          <h2>Proses Kandidat Sales?</h2>
          <p>
            Setelah diproses, data <strong>{candidate.fullName}</strong> akan masuk tahap
            interviewing di sales dan recruiter.
          </p>
        </div>

        <div className="recruiter-process-summary">
          <span>Perubahan tahap</span>
          <strong>TO DO {'->'} INTERVIEWING</strong>
        </div>

        <div className="sales-process-modal-section">
          <div className="sales-process-modal-intro">
            <strong>{candidate.fullName}</strong>
            <span>{candidate.appliedRole}</span>
          </div>

          <div className="sales-process-modal-grid">
            <label className="sales-process-field">
              Schedule Interview
              <input
                required
                name="interviewSchedule"
                type="datetime-local"
                value={processForm.interviewSchedule}
                onChange={onChange}
              />
              <small>Tentukan jam interview sebelum kandidat masuk tahap interviewing.</small>
            </label>
            ...
            <label className="sales-process-field">
              ...
              <input
                ...
                type="url"
                value={processForm.interviewLink}
                onChange={onChange}
              />
              <small>Masukkan link meeting yang akan dipakai untuk sesi interview.</small>
            </label>
          </div>
        </div>
...
interface SalesCandidateRemoveModalProps {
  candidate: SalesCandidateRecord | null;
  onClose: () => void;
  onConfirm: () => void;
}

function SalesCandidateRemoveModal({
  candidate,
  onClose,
  onConfirm,
}: SalesCandidateRemoveModalProps) {
  if (!candidate) {
    return null;
  }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Konfirmasi hapus kandidat sales"
      >
        <div className="modal-header recruiter-process-modal-header">
          <h2>Hapus Data Sales?</h2>
          <p>
            Apakah Anda yakin ingin menghapus data <strong>{candidate.fullName}</strong> dari
            sales?
          </p>
          <p className="muted-text">
            Kandidat akan dikembalikan ke tahap TO DO di recruiter.
          </p>
        </div>
...
function SalesPage({ view }: SalesPageProps) {
  const dispatch = useAppDispatch();
  const user = useAppSelector((state) => state.auth.user);
  const interviewPagination = useAppSelector((state) => state.interviews.pagination);
  const [candidates, setCandidates] = useState<SalesCandidateRecord[]>(() => readSalesCandidates());
  ...
  useEffect(() => {
    writeSalesCandidates(candidates);
  }, [candidates]);

  const syncRecruiterMirror = useCallback(async () => {
    const recruiterResponse = await fetchRecruiterCandidates('all');
  ...
  const closeProcessModal = () => {
    ...
    setProcessFormValues(emptyProcessFormValues);
  };
  ...
  const handleEdit = (candidate: SalesCandidateRecord) => {
  ...
  const handleEditFromDetail = (candidate: SalesCandidateRecord) => {
    closeDetailModal();
  ...
  const handleRemove = async () => {
    ...
    try {
      await deleteSalesCandidate(removeCandidateId);
      await Promise.all([loadCandidates(), syncRecruiterMirror()]);
      closeRemoveModal();
    } catch (error) {
      window.alert(error instanceof Error ? error.message : 'Gagal menghapus kandidat sales.');
    }
  };
  ...
  const handleProcessClick = (candidateId: string) => {
    ...
    setProcessFormValues({
      interviewLink: selectedCandidate?.interviewLink ?? '',
      interviewSchedule: selectedCandidate?.interviewSchedule ?? '',
    });
    setProcessCandidateId(candidateId);
  };

  const handleProcessFieldChange = (event: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = event.target;

    setProcessFormValues((current) => ({
      ...current,
      [name]: value,
    }));
  };
  ...
  const handleProcess = async () => {
    ...
    if (!processFormValues.interviewSchedule || !processFormValues.interviewLink.trim()) {
      window.alert('Schedule interview dan link interview wajib diisi sebelum data diproses.');
      return;
    }

    const processedCandidate = candidates.find((candidate) => candidate.id === processCandidateId);
    ...
    try {
      await processSalesCandidate({
        candidateId: processedCandidate.candidateId ?? processedCandidate.id,
        scheduledAt: processFormValues.interviewSchedule,
        meetingLink: processFormValues.interviewLink.trim(),
        notes: processedCandidate.summary || 'Diteruskan dari proses sales.',
        ownerUserId: user?.id,
      });
      await Promise.all([loadCandidates(), syncRecruiterMirror()]);
      await dispatch(
        fetchInterviews({
          ...
          limit: interviewPagination.limit,
        })
      );
      closeProcessModal();
    } catch (error) {
      window.alert(error instanceof Error ? error.message : 'Gagal memproses kandidat sales.');
    }
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!uploads.cv && !hasExistingCv) {
      window.alert('CV kandidat wajib tersedia sebelum data disimpan.');
      return;
    }
  ...
  const processedBySales = scopedCandidates.filter(
    (candidate) => candidate.stage === 'INTERVIEWING'
  ).length;
  const pageTitle =
    view === 'todo' ? 'Sales To do Candidates' : 'Sales Candidate Database';
  const pageDescription =
    view === 'todo'
      ? 'Sub menu sales untuk melihat kandidat handoff recruiter yang masih berstatus To do.'
      : 'Sub menu sales untuk melihat seluruh kandidat hasil handoff recruiter.';
  const panelTitle =
    view === 'todo' ? 'Database Kandidat To do Sales' : 'Database Kandidat Sales';
  const panelDescription =
    view === 'todo'
      ? 'Menampilkan hanya kandidat sales dengan tahap To do berdasarkan filter aktif.'
  ...
  const salesColumns: DataTableColumn[] = [
    ...
    { key: 'interviewSchedule', label: 'Schedule Interview' },
    { key: 'interviewLink', label: 'Link Interview' },
    { key: 'documents', label: 'Dokumen' },
    { key: 'summary', label: 'Catatan' },
    { key: 'createdAt', label: 'Masuk Sales' },
    { key: 'actions', label: 'Aksi' },
  ];

  const renderCandidateActions = (candidate: SalesCandidateRecord) => (
    ...
    <>
      ...
      {errorMessage ? (
        <div className="empty-block">
          <strong>Sinkronisasi sales belum berhasil.</strong>
          <p>{errorMessage}</p>
        </div>
      ) : null}
      ...
      <section className="summary-grid summary-grid-compact">
        <article className="summary-card">
          ...
          <small>Data kandidat yang sudah masuk ke menu sales</small>
        </article>
        <article className="summary-card">
          <span>CV Lengkap</span>
          <strong>{candidatesWithCv}</strong>
          <small>Kandidat sales yang sudah memiliki file CV</small>
        </article>
        <article className="summary-card">
        ...
        <article className="summary-card">
          <span>Sudah Diproses Sales</span>
          <strong>{processedBySales}</strong>
          <small>Kandidat yang sudah masuk tahap interviewing di sales</small>
        </article>
      </section>
      ...
      <DataTable
        ...
        emptyMessage={
          isLoading
            ? 'Memuat data sales dari backend...'
            : 'Belum ada kandidat sales yang cocok dengan filter saat ini.'
        }
        headerActions={
        ...
        renderRow={(candidate) => (
          <tr key={candidate.id}>
            ...
            <td>{formatInterviewSchedule(candidate.interviewSchedule ?? '')}</td>
            <td>
              {candidate.interviewLink ? (
                <a href={candidate.interviewLink} target="_blank" rel="noreferrer">
                  Open Link
            ...
            <td>
              <div className="recruiter-note-wrapper">
                <p className="recruiter-note">
                  {candidate.summary || 'Belum ada catatan sales.'}
                </p>
      ...
      <SalesCandidateProcessModal
        candidate={processCandidate}
        processForm={processFormValues}
        onChange={handleProcessFieldChange}
        onClose={closeProcessModal}
        onConfirm={handleProcess}
      />

      <SalesCandidateRemoveModal
